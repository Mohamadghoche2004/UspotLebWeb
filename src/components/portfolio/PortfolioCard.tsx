...
export function PortfolioCard({ project, onClick, compact = false }: PortfolioCardProps) {
  return (
    ...
    >
      ...
      <div className={cn('border-t border-border-subtle', compact ? 'p-5' : 'p-6')}>
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          {project.technologies.slice(0, compact ? 2 : 3).map((tech) => (
            <span
              key={tech}
              className="rounded-full bg-foreground/10 px-2 py-1 text-xs text-muted-foreground"
            >
              {tech}
            </span>
          ))}
        </div>
        <p className="mt-3 text-sm font-semibold text-accent-cyan">{project.results}</p>
      </div>
    </motion.article>
  )
}
